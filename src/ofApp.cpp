...
void ofApp::setup()
{
	...
	RecordAndPlayGM = new Button(ofGetWindowWidth() / 2 - 500, ofGetWindowHeight() / 2 - 450, 300, 300, "images/recordAndPlay.png", "sounds/arcade.mp3");
	MultiplayerGM = new Button(ofGetWindowWidth() - 192, ofGetWindowHeight() / 2 - 385, 150, 150, "images/multiplayerButtonImage.png", "sounds/arcade.mp3");
	LightningGM = new Button(ofGetWindowWidth()/2 - 500, ofGetWindowHeight() - 235, 250, 200, "images/lightning.png", "sounds/arcade.mp3");
...
void ofApp::mousePressed(int x, int y, int button)
{
	...
	if (gameState == StartUp)
	{
		...
		{
			RecordAndPlayGM->playSound();
			idle = false;
			gameState = RecordAndPlay;
		}
		else if (MultiplayerGM->wasPressed())
		{
			MultiplayerGM->playSound();
			gameState = Multiplayer;
			GameReset();
		}
		else if(LightningGM->wasPressed()){
			LightningGM->playSound();
			gameState = Lightning;
			GameReset();
		}
	}
